...
//mod sun_tower_creator;
//type PartCreator = sun_tower_creator::SunTowerCreator;

//mod banana_creator;
//type PartCreator = banana_creator::BananaCreator;

//mod cannyminx_creator;
//type PartCreator = cannyminx_creator::CannyminxCreator;

//mod octopus_creator;
//type PartCreator = octopus_creator::OctopusCreator;
      ...
      Event::WindowEvent { event, .. } => match event {
        ...
        WindowEvent::KeyboardInput { device_id, input, is_synthetic } => {
          ...
          match input.virtual_keycode {
            Some(winit::event::VirtualKeyCode::W) => renderer.forward(input.state == Pressed),
            Some(winit::event::VirtualKeyCode::S) => renderer.back(input.state == Pressed),
            Some(winit::event::VirtualKeyCode::A) => renderer.left(input.state == Pressed),
            Some(winit::event::VirtualKeyCode::D) => renderer.right(input.state == Pressed),
            Some(winit::event::VirtualKeyCode::LControl) => renderer.down(input.state == Pressed),
            Some(winit::event::VirtualKeyCode::Space) => renderer.up(input.state == Pressed),
...
impl Renderer {
  pub fn new<D: GlDisplay>(gl_display: &D) -> Self {
    unsafe {
        ...
        for (index, mut cc) in contours {
          ...
          for j in 0..ex.len() {
            if let Err(msg) = ex[j].save_to_stl(
              &std::path::Path::new("extruded").join(format!("extruded_{i}_{index}_{j}.stl")),
            ) {
              println!("{}", msg);
            }
          }
          ...
          if let Err(msg) = cc
            .save_to_dxf(&std::path::Path::new("contours").join(format!("contour_{i}_{index}.dxf")))
          {
            println!("{}", msg);
          }
        }
      ...
      let pos_attrib =
        gl.GetAttribLocation(program, b"position\0".as_ptr() as *const _) as gl::types::GLuint;
      let center_attrib =
        gl.GetAttribLocation(program, b"center\0".as_ptr() as *const _) as gl::types::GLuint;
      let color_attrib =
