...
impl SunTowerCreator {
  pub fn new() -> Self {
    let basic_angle = std::f32::consts::PI * 2.0 / 6.0;
    ...
    let ball_radius = 14.5;

    let split_angle = diamond_angle + 1.35 / ball_radius;
    ...
    Self {
      ...
      split2_cos: (split_angle + 1.5 / ball_radius).cos(),
      ball_radius,
    }
  }
  ...
  pub fn get_part_index_impl(&self, pos: Point, current_normal: usize) -> PartIndex {
    ...
    if (index & 1) != 0 {
      ...
      match_axis(&mut index, 6, self.axis[1].rotate(self.axis[0], self.basic_angle * 5.0));
    }
    if (index & 2) != 0 {
      ...
      match_axis(&mut index, 9, self.axis[0].rotate(self.axis[1], self.basic_angle * 5.0));
    }
    if (index & 4) != 0 {
      ...
      match_axis(&mut index, 12, self.axis[0].rotate(self.axis[2], self.basic_angle * 5.0));
    }
    if (index & 8) != 0 {
      match_axis(&mut index, 13, self.axis[0].rotate(self.axis[3], self.basic_angle));
      match_axis(&mut index, 14, self.axis[0].rotate(self.axis[3], self.basic_angle * 3.0));
      match_axis(&mut index, 15, self.axis[0].rotate(self.axis[3], self.basic_angle * 5.0));
    }
    return index;
  }
