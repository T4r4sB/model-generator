...
pub struct ZmeyGorynychKitesCreator {
  axis: Vec<Point>,
  axis1: Vec<Point>,
  axis2: Vec<Point>,
  normals: Vec<Point>,
  ...
  extra_split: FxHashMap<PartIndex, Vec<(Point, usize)>>,
...
impl ZmeyGorynychKitesCreator {
  pub fn new() -> Self {
    let crot = (PI * 4.0 / 9.0).cos();
    let edge = crot / (1.0 - crot);
    let sedge = (1.0 - sqr(edge)).sqrt();
    ...
    let axis: Vec<_> = [
      ...
      Point { x: 0.0, y: -sedge, z: -edge },
      Point { x: -tr * sedge, y: 0.5 * sedge, z: -edge },
      Point { x: tr * sedge, y: 0.5 * sedge, z: -edge },
    ]
    .into_iter()
    .map(Point::norm)
    ...
    let e1 = dot(axis[0], axis[1]);
    let e2 = dot(axis[0], axis[5]);
    println!("edge={edge},e1={e1}, e2={e2}");

    let mut normals: Vec<_> = [
      (axis[0] + axis[3]).norm(),
      (axis[1] + axis[3]).norm(),
      (axis[2] + axis[3]).norm(),
      (axis[0] + axis[1] + axis[6]).norm(),
      (axis[1] + axis[2] + axis[4]).norm(),
      (axis[2] + axis[0] + axis[5]).norm(),
    ]
    .into_iter()
    .map(Point::norm)
    .collect();
    ...
    println!("n0={}, n1={}", dot(normals[0], normals[1]), dot(normals[3], normals[4]));

    let c_max = dot(axis[0], axis[3]);
    let c_min = dot(axis[4], axis[5]);
    ...
    let r = f32::max(23.0, 4.8 / f32::max(0.1, a_max - a_min));
    ...
    let mut extra_split = FxHashMap::<PartIndex, Vec<_>>::default();
    ...
    for i1 in 0..axis.len() {
      ...
      for i2 in 0..axis.len() {
        ...
        for i3 in 0..axis.len() {
          ...
          extra_split
            .entry(1 << i1 | 1 << i2 | 1 << i3)
            .or_default()
            .push((a3.rotate(center, PI), axis.len()));
          extra_split
            .entry(1 << i1 | 1 << i3)
            .or_default()
            .push((a3.rotate(center, ta * 2.0), axis.len() + i2));
          extra_split
            .entry(1 << i2 | 1 << i3)
            .or_default()
            .push((a3.rotate(center, -ta * 2.0), axis.len() + i1));
    ...
    for i1 in 0..axis.len() {
      ...
      for i2 in 0..axis.len() {
        ...
        if !has {
          ...
          extra_split.entry(1 << i1 | 1 << i2).or_default().push((a3, 0));
        }
      }
    }
  ...
  pub fn get_part_index_impl(&self, pos: Point, current_normal: usize) -> PartIndex {
    ...
    if let Some(splits) = self.extra_split.get(&(index & ((1 << self.axis.len()) - 1))) {
      for (si, (axis, i)) in splits.iter().enumerate() {
        if !match_axis(&mut index, *axis, if *i == 0 { si + self.axis.len() } else { *i }) {
          return 0;
        }
      }
    }
    ...
    if index.count_ones() <= 3 || self.corners.contains(&index) || index.count_ones() == 2 {
      if n_dists[0].0 < 3.7 {
