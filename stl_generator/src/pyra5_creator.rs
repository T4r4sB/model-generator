...
impl Pyra5Creator {
  ...
  pub fn get_part_index_impl(&self, pos: Point, current_normal: usize) -> PartIndex {
    ...
    if r < inner_r {
      ...
      let mut index = if pos.z > 0.0 { 33 } else { 34 };
      if pos.z.abs() < 0.15 {
        return 0;
      }
    ...
    let mut match_all = || -> Option<()> {
      ...
      if index == 3 {
        match_axis(pos, &mut index, 5, self.a01_l)?;
        match_axis(pos, &mut index, 6, self.a01_r)?;
      }
      if index == 5 {
      ...
      if index == 17 {
        match_axis(pos, &mut index, 5, self.a04_l)?;
        match_axis(pos, &mut index, 6, self.a04_r)?;
      }
      if index == 6 {
        match_axis(pos, &mut index, 5, self.r12_l)?;
        match_axis(pos, &mut index, 6, self.r12_r)?;
      }
      if index == 12 {
      ...
      if index == 18 {
        match_axis(pos, &mut index, 5, self.r41_l)?;
        match_axis(pos, &mut index, 6, self.r41_r)?;
      }
