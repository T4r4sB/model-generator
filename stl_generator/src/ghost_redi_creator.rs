...
impl RediCreator {
  pub fn new() -> Self {
    ...
    .map(|p| {
      Point { x: -p.x, y: p.y, z: p.z }
        .rotate(Point { x: 1.0, y: 1.0, z: 1.0 }.norm(), PI * 0.3)
        .rotate(Point { x: 1.0, y: -2.0, z: -1.0 }.norm(), PI * 0.55)
    })
    .collect();
    ...
    let fe1 = dot(axis[0], axis[4]);
    let fe2 = dot(axis[1], axis[7]);
    let se1 = dot(axis[0], axis[2]);
    let se2 = dot(axis[1], axis[3]);

    let rot1 = ((fe1 - sqr(edge)) / (1.0 - sqr(edge))).acos();
    let rot2 = ((fe2 - sqr(edge)) / (1.0 - sqr(edge))).acos();

    let axis_seq =
      vec![(0, PI), (3, rot1 * 2.0), (2, rot2 * 2.0), (5, -rot1), (4, -rot2), (7, -rot1)];

    let maximal_angle0 = fe1.acos() * 0.5;
    let maximal_angle1 = se2.acos() * 0.5;
    let minimal_angle = edge.acos() * 0.5;
    let sphere_r0 = 4.0 / (maximal_angle0 - minimal_angle);
    let sphere_r1 = 4.0 / (maximal_angle1 - minimal_angle);
    ...
    Self { axis, axis1, axis2, normals, groove0, groove1, axis_pos, axis_neg, axis_seq }
  }
  ...
  pub fn get_sticker_index(&self, pos: crate::points2d::Point, current_normal: usize) -> PartIndex {
    ...
    let sz = last_groove + 2.19;
    if pos.x.abs() > sz + 4.0 || pos.y.abs() > sz + 4.0 {
      return 0;
    }
    ...
    let pos = n.scale(sz) + n1.scale(pos.x) + n2.scale(pos.y);
  ...
  pub fn get_part_index_impl(&self, pos: Point, current_normal: usize) -> PartIndex {
    ...
    let hole = 2.28;
    if pos.x + pos.y + pos.z < 6.0 + hole {
      return 0;
    }
    if pos.x > hole + 12.0 || pos.y > hole + 12.0 || pos.z > hole + 12.0 {
      return 0;
    }
    if pos.x > hole + 2.0 && pos.y > hole + 2.0 && pos.z > hole + 2.0 {
      return 0;
    }
    if pos.x > hole + 6.0 && pos.y > hole + 6.0 {
      return 0;
    }
    if pos.y > hole + 6.0 && pos.z > hole + 6.0 {
      return 0;
    }
    if pos.z > hole + 6.0 && pos.x > hole + 6.0 {
      return 0;
    }
    if pos.x > hole + 2.0 && pos.y > hole + 2.0 && pos.z > 1.0 && pos.z < hole + 1.0 {
      return 0;
    }
    if pos.y > hole + 2.0 && pos.z > hole + 2.0 && pos.x > 1.0 && pos.x < hole + 1.0 {
      return 0;
    }
    if pos.z > hole + 2.0 && pos.x > hole + 2.0 && pos.y > 1.0 && pos.y < hole + 1.0 {
      return 0;
    }
    return 1;*/
    ...
    if !thick {
      ...
      let r_shift = if current_normal < self.normals.len() { 0.04 } else { 0.0 };

      let mut in_sr = |a, b, d| {
        let r = 0.096 * d - r_shift;
